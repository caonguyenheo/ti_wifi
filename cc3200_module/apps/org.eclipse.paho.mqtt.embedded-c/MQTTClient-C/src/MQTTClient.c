...
int keepalive(Client* c)
{
    ...
    if (expired(&c->ping_timer))
    {
        if (!c->ping_outstanding)
        {
            ...
            if (keepalive_count>=mqtt_noKL_toreset){ // KL many times but no Yield
                for(i=0;i<20;i++)
                    UART_PRINT("MqttKReboot");
                system_reboot();
            }
            keepalive_count++;
            yield_count=0;
...
int MQTTConnect(Client* c, MQTTPacket_connectData* options)
{
    ...
    c->keepAliveInterval = (45*3);// Manually increase KL Interval set to server
