...
int httpServerOn=1;
int httpServerOperationTime=0;
extern int g_exit_loop_http ;
void RunHttpServer(void)
{
  ...
  while(1)
  {
    ...
    while (httpServerOn!=1){
        osi_Sleep(1000);
        httpServerOperationTime=0;
    }
	if(g_exit_loop_http  == 1)
	{
		 HttpDebug("exit http");
		break;
	}
    if (httpServerOperationTime==0)
    	httpServerOperationTime=1;
    if ((httpServerOperationTime>(10*REGISTRATION_TIMEOUT_S)) && (g_input_state != LED_STATE_FTEST))
    	break;
    if(g_state.listenSocket < 0)
    {
